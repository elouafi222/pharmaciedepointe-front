...
function CyclesEnRetard() {
  ...
  const fetchOrdo = async () => {
    try {
      ...
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/ordonnance/enRetardCycles?page=${currentPage}&search=${filters.search}&date=${filters.date}&numero=${filters.numero}`,
        {
          headers: {
  ...
  const [isLoadingCheck, setIsLoadingCheck] = useState(null);
  const handleCheck = async (ordonnanceId, numero) => {
    try {
      setIsLoadingCheck(ordonnanceId);
      await axios.put(
        `${process.env.REACT_APP_API_URL}/ordonnance/addOrdonnanceCollab/${ordonnanceId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
      ...
      fetchOrdo();
      toast.success(`L'ordonnance ORD-${numero} vous a été assignée.`);
    } catch (error) {
      // console.error("Error assigning collaborateur:", error);
      toast.error(
        `Erreur lors de l'assignation du collaborateur. Veuillez réessayer plus tard.`
      );
    } finally {
      setIsLoadingCheck(null);
    }
  };
  ...
  return (
    <div className="flex ">
      ...
      <div className="h-screen flex-1 p-1 md:p-7  overflow-y-scroll">
        <Navbar page=" Ordonnances avec des renouvellements en retard" />
        <Explain
          text="Cette page affiche uniquement les ordonnances dont les renouvellements
        sont retardés, en fonction de la date de renouvellement spécifiée."
        />
                ...
                ordonnances.map((ordonnance, index) => (
                  ...
                  >
                    ...
                    <th className="px-3 py-4 ">
                      <span className="flex justify-center items-center px-2 py-1 text-sm rounded-lg bg-red-500 text-white">
                        {ordonnance.cycles.length}
                      </span>
                    </th>
                        ...
                        <>
                          <label>
                            <input
                              ...
                              type="checkbox"
                              onChange={() =>
                                setSelectedOrdonnance(ordonnance._id)
                              }
                            />
                            Prendre la responsabilité
                          </label>
                          <button
                            className="bg-green-500 px-3 py-1 rounded-lg text-white hover:bg-green-700 cursor-pointer"
                            onClick={() =>
                              handleCheck(ordonnance._id, ordonnance.numero)
                            }
                            disabled={
                              !selectedOrdonnance ||
                              selectedOrdonnance !== ordonnance._id
                            }
                          >
                            Confirmer
                          </button>
