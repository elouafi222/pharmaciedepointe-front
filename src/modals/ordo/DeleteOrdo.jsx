...
function DeleteOrdo({ show, handleClose, user, refreshOrdo, ordonnance }) {
  ...
  const deleteMessage = async (e) => {
    ...
    try {
      ...
      toast.success("L'ordonnance a été supprimée avec succès.");
      refreshOrdo();
      handleClose(false);
    } catch (error) {
                  ...
                  <div className="flex flex-row justify-between rounded-lg items-center p-3 mb-4 bg-gradient-to-r from-slate-950 to-slate-800">
                    <span className="flex items-center">
                      ...
                      <h1 className="text-sm md:text-lg text-white">
                        Supprimer ORD-{ordonnance.numero}
                      </h1>
                    </span>
                    <CircleX
                      onClick={() => handleClose(false)}
