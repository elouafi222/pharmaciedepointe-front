...
import { CircleAlert, CircleX, LoaderCircle, Trash } from "lucide-react";
...
function DeleteManyMessage({
  ...
  const deleteMessage = async (e) => {
    ...
    try {
      setIsLoading(true);
      await axios.delete(
        `${process.env.REACT_APP_API_URL}/message/deleteMany`,
        {
          headers: {
      ...
      toast.success("Les messages ont été supprimés avec succès.");
      refreshMessages();
      handleClose(false);
    } catch (error) {
      toast.error(
                ...
                <div className="flex flex-col">
                  <div className="flex flex-row justify-between rounded-lg items-center p-3 mb-4 bg-gradient-to-r from-slate-950 to-slate-800">
                    <span className="flex items-center">
                      <Trash size={20} className="text-white me-2" />
...
export default DeleteManyMessage;
